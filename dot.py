...
parser.add_argument('-r', '--resource-group', dest='resourceGroup', type=str, help='The resource group of the resource.')
parser.add_argument('-s', '--suffix', type=str, default='sea', help='The suffix (region code) of the resource.')
...
def updateNSG(
    ...
    info = f'{accessType.upper()}: {sourceIP} -> {destinationIP} [{name}]'
...
def main():
    ...
    if task in ('open', 'close'):
        ...
        updateNSG(task=args.task, prefix=args.prefix, suffix=args.suffix, sourceIP=sourceIP)
